...
//    (또는 prisma/seed.ts 파일 수정 후 npx prisma db seed)
//
// 3. 프론트엔드 타입 정의 확인
//    - src/types/quizTypes.ts의 TopicId 타입에 토픽 추가 확인
//    - src/lib/translations/ko.ts에 한글 번역 추가
//    - src/lib/translations/en.ts에 영문 번역 추가
//
// 4. 검증
...
async function importQuestion(
  ...
  // Concept 매칭
  let conceptConnect: { id: string }[] | undefined;
  if (q.concept) {
    const matched = matchConcept(q.concept, q.topic);
    if (matched) {
      conceptConnect = [{ id: matched.id }];
      stats.conceptsMatched++;
    } else {
      stats.conceptsUnmatched++;
      console.log(`    ⚠ concept unmatched: "${q.concept}"`);
    }
  }
  ...
  const created = await prisma.question.create({
    data: {
      ...
      ...(conceptConnect && {
        concepts: { connect: conceptConnect },
      }),
    },
  });
  return created.id;
...
async function main() {
  ...
  // Load seed concepts for tagging
  ...
  for (const { file, data } of fileData) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (!dryRun) {
        try {
          const id = await importQuestion(q, stats);
  ...
  console.log(`  Concepts matched:   ${stats.conceptsMatched}`);
  console.log(`  Concepts unmatched: ${stats.conceptsUnmatched}`);
