...
- 개념 태깅: 문제별 CS 개념 분류 및 홈페이지 쇼케이스
...
홈페이지에서 원하는 모드를 선택할 수 있습니다.

**오늘의 퀴즈** - 매일 새로운 15문제 세트로 도전합니다.

**랜덤 퀴즈** - 전체 주제에서 무작위로 문제를 출제합니다.

**주제별 퀴즈** - 특정 주제를 선택하여 문제를 풀 수 있습니다.

퀴즈를 풀 때는 문제를 읽고 필요하면 힌트를 확인한 후 답을 선택합니다. 제출 후 해설을 볼 수 있으며, 오류가 있다면 신고할 수 있습니다. 퀴즈 중에는 맞은 문제 수와 소요 시간이 실시간으로 표시됩니다.

퀴즈를 완료하면 점수가 자동으로 저장되고 리더보드에 등록됩니다. 상단 메뉴에서 언어를 전환할 수 있으며 변경사항이 즉시 반영됩니다.
...
### 다국어 지원
Context API를 통해 한국어와 영어를 실시간으로 전환할 수 있습니다. 선택한 언어는 localStorage에 저장되어 다시 방문할 때 유지됩니다.

### 힌트 시스템
각 문제마다 1개의 힌트를 제공합니다. 답변 제출 전에만 확인할 수 있으며, 통계 버튼과 같은 줄에 표시되어 공간을 효율적으로 사용합니다.

### 해설 아코디언
모든 답안에 대한 상세 해설을 제공합니다. 정답은 자동으로 펼쳐지며, 오답도 클릭하여 확인할 수 있습니다.

### 개념 쇼케이스
홈페이지 하단에는 토픽별 CS 개념 태그가 표시됩니다. Intersection Observer를 사용한 무한 스크롤로 토픽별 3개씩 배치 로딩합니다.

### 문제 신고
문제 풀이 중 오답, 문제 오류, 선지 오류, 해설 오류, 번역 오류, 기타 6개 카테고리로 신고할 수 있습니다. 신고 데이터는 카테고리와 함께 저장됩니다.

### 리더보드
일일 퀴즈 완료 후 리더보드에 등록됩니다. 점수는 (정답 수 × 100) - (소요 시간(초) / 10) 으로 계산되며, 실시간으로 순위가 업데이트됩니다.
